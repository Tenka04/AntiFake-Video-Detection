  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white tracking-tight">AI Video Detection Dashboard</h1>
          <p className="text-gray-400 mt-1">Overview of your video analysis activity.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat, index) => (
          <div key={index} className="bg-surface/40 backdrop-blur-md border border-white/5 rounded-2xl p-6 flex flex-col hover:border-white/10 transition-colors">
      ...
      <div className="bg-surface/30 border border-white/5 rounded-2xl p-6 backdrop-blur-sm h-96 flex flex-col">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center opacity-70">
          ...
          <p className="text-sm text-gray-600 mt-1">Videos you analyze will appear here.</p>
        </div>
      </div>
    </div>
  );
